...
public plugin_init()
{
	register_plugin("Map manager", "3.2", "Abhinash")
...
public Command_Timeleft(id)
{
	...
	{
		new szText[128], len
		len = formatex(szText, charsmax(szText), "Until map end left ")
		if (iWinLimit)
		{
			new iLeftWins = iWinLimit - max(g_iTeamScore[0], g_iTeamScore[1])
			new szWins[16]; get_ending(iLeftWins, "wins", "wins", "wins", szWins, charsmax(szWins))
			len += formatex(szText[len], charsmax(szText) - len, "%d %s", iLeftWins, szWins)
		}
		if (iWinLimit && iMaxRounds)
		{
			len += formatex(szText[len], charsmax(szText) - len, " or ")
		}
		if (iMaxRounds)
		{
			new iLeftRounds = iMaxRounds - g_iTeamScore[0] - g_iTeamScore[1]
			new szRounds[16]; get_ending(iLeftRounds, "rounds", "rounds", "rounds", szRounds, charsmax(szRounds))
			len += formatex(szText[len], charsmax(szText) - len, "%d %s", iLeftRounds, szRounds)
		}
...
public Command_RockTheVote(id)
{
	...
	if (iVotes <= 0)
	{
		...
		{
			StartVote(0)
			client_print_color(0, print_team_default, "%s^1 Starting early voting for next map.", PREFIX)
		}
		else
		{
			SetNewRoundVote()
	...
	new szVote[16];	get_ending(iVotes, "votes", "votes", "votes", szVote, charsmax(szVote))
...
{
	new szText[64]; formatex(szText, charsmax(szText), "Found some maps:")
	...
	formatex(szText, charsmax(szText), "Next")
	...
	if (nominate_index)
	{
		...
		if (id == eNomInfo[n_Player])
		{
			...
			if (g_iLastDenominate[id] + MIN_DENOMINATE_TIME <= iSysTime)
			{
				...
				client_print_color(0, id, "%s^3 %s^1 has denominated map^3 %s^1.", PREFIX, szName, map)
				return 2
			}
			client_print_color(id, print_team_default, "%s^1 Don't spam with removing a nomination.", PREFIX)
			return 0
		}
		client_print_color(id, print_team_default, "%s^1 This map is already nominated.", PREFIX)
		return 0
	}
	
	if (g_iNominatedMaps[id] >= NOMINATED_MAPS_PER_PLAYER)
	{
		client_print_color(id, print_team_default, "%s^1 You can't nominate any more maps.", PREFIX)
		return 0
	}
...
public Event_NewRound()
{
	...
	if ((g_bVoteFinished && (g_bRockVote && get_pcvar_num(g_pCvars[ROCK_CHANGE_TYPE]) == 1 || get_pcvar_num(g_pCvars[CHANGE_TYPE]) == 1 || get_pcvar_num(g_pCvars[LAST_ROUND]))) || get_cvar_num("mp_timelimit") == 0)
	{
		Intermission()
...
public VoteMenu(id)
{
	...
	iLen = formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y%s:^n^n", g_bPlayerVoted[id] ? "Voting Result" : "Choose your next map")
	...
	new szSec[16]; get_ending(g_iTimer, "seconds", "seconds", "seconds", szSec, charsmax(szSec))
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "^n\dLeft \r%d\d %s", g_iTimer, szSec)
...
FinishVote()
{
	...
	else
	{
		...
		else
		{
			...
			new szMin[16]; get_ending(iMin, "minutes", "minutes", "minutes", szMin, charsmax(szMin))
			
			client_print_color(0, print_team_default, "%s^1 Current map has been extended to^3 %d^1 %s.", PREFIX, iMin, szMin)
