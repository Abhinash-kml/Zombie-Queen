...
native RegisterPointsShopWeapon(const name[], price, team)
...
public plugin_init()
{
	...
	RegisterHam(Ham_TraceAttack, "worldspawn", "OnTraceAttackWorld")
	RegisterHam(Ham_TraceAttack, "player", "OnTraceAttackPlayer")
	...
	g_item = RegisterPointsShopWeapon("AK-47 Paladin", 120, 1 | 2)
}
...
{
	register_native("GetPaladin", "native_get_paladin", 1)
	register_native("SetPaladinModels", "native_set_paladin", 1)
}

public native_get_paladin(id)
{
	return Get_BitVar(g_Had_Base, id)
}

public native_set_paladin(id)
{
	SwitchModel(id)
}

public OnPointsShopWeaponSelected(id, itemid, playername[])
{
	if (itemid == g_item)
	{
		Get_Base(id)

		set_hudmessage(random(256), random(256), random(256), -1.00, 0.70, 0, 0.0, 3.0, 2.0, 1.0, -1)
		ShowSyncHudMsg(0, g_hudsync, "%s bought AK-47 Paladin", playername)
	}
}

public OnInfectedPost(victim, infector, class)
{
	if (Get_BitVar(g_Had_Base, victim)) Remove_Base(victim)
}
...
public Register_HamBot(id)
{
	...
	RegisterHamFromEntity(Ham_TraceAttack, id, "OnTraceAttackWorld")
	RegisterHamFromEntity(Ham_TraceAttack, id, "OnTraceAttackPlayer")
}
...
public OnTraceAttackWorld(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
...
public OnTraceAttackPlayer(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
