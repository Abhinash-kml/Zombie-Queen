...
#define M32_CLIP 10000
#define M32_BPAMMO 3200000
#define M32_DAMAGE 350.0
...
public plugin_init()
{
	...
	g_item = RegisterPointsShopWeapon("Milkor MG32-MGl", 120, 1 | 2)
...
public OnPointsShopWeaponSelected(id, itemid, playername[])
{
	if (itemid == g_item)
	{
		...
		ShowSyncHudMsg(0, g_hudsync, "%s bought Milkor MG32-MGL", playername)
	}
}

public OnInfectedPost(victim, infector, class)
{
	if (g_has_M32[victim]) g_has_M32[victim] = false
}
	...
	{	
		...
		if (equal(classname, "M32_grenade"))
		{
			...
			while((a = find_ent_in_sphere(a,originF,300.0)) != 0) {
				if (a!=owner&& a!=ptr&&pev(a,pev_takedamage)!=DAMAGE_NO) ExecuteHamB(Ham_TakeDamage, a ,owner ,owner,  M32_DAMAGE, DMG_BLAST)
