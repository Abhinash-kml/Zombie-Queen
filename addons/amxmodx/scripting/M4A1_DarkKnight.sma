...
public plugin_init()
{
	...
	g_item = RegisterPointsShopWeapon("M4A1 Darknight", 170, 1 | 2)
...
public OnPointsShopWeaponSelected(id, itemid, playername[])
{
	if (itemid == g_item)
	{
		...
		ShowSyncHudMsg(0, g_hudsync, "%s bought M4A1 Darknight", playername)
	}
}
...
public Get_Base(id)
{
	...
	set_hudmessage(random_num(0, 255), random_num(0, 255), random_num(0, 255), -1.00, 0.70, 0, 0.0, 3.0, 2.0, 1.0, -1)
	ShowSyncHudMsg(0, g_hudsync, "%s now has M4A1 Darknight", name)
}
