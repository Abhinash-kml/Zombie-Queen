...
public plugin_init()
{
	register_plugin("Janus 7", "2.0", "Abhinash")
	...
	g_item = RegisterPointsShopWeapon("Janus 7", 120, 1 | 2)
...
public OnPointsShopWeaponSelected(id, itemid, playername[])
{
	if (itemid == g_item)
	{
		...
		ShowSyncHudMsg(0, g_hudsync, "%s bought Janus 7", playername)
	}
}

public OnInfectedPost(victim, infector, class)
{
	if (Get_BitVar(g_Had_Janus7, victim)) Remove_Janus7(victim)
}
