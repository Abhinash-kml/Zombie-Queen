...
public plugin_init()
{
	...
	register_concmd("amx_unban", "CmdUnban", -1, _, -1);
	register_concmd("amx_banlist", "CmdBanList", -1, _, -1);
	register_srvcmd("amx_addbanlimit", "CmdAddBanLimit", -1, _);
...
{
	static iFile
	iFile = fopen("addons/amxmodx/configs/Admins.ini", "r")
	if (iFile)
	{
		static cLine[161]
		while (!feof(iFile))
		{
			...
			{
				parse(cLine, g_AdminNames[g_AdminsCount], charsmax(g_AdminNames), g_AdminPasswords[g_AdminsCount], charsmax(g_AdminPasswords), g_AdminFlags[g_AdminsCount], charsmax(g_AdminFlags), g_AdminSkinFlags[g_AdminsCount], charsmax(g_AdminSkinFlags))
				g_AdminsCount++
			}		
		}
		fclose (iFile)
	...
	while (i < g_AdminsCount)
	{
		if (equali(g_AdminNames[i], g_playername[id]))
		{
			...
			if (equali(g_cPassword[id], g_AdminPasswords[i]))
			{
				...
				log_amx("Login: ^"%s^" became an admin. [ %s ] ", g_playername[id], g_cAdminFlag[id], g_cAdminSkinFlag, g_cPlayerAddress[id])
				return PLUGIN_CONTINUE
			}
			else
			{
...
{
	if (g_bAdmin[client] && AdminHasFlag(client, 'e'))
	{
		static arg[128];
